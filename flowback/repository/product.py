...
async def getProductList(db: AsyncSession, *, pid: str = None, page: int = 1, stype: str = None, keyword: str = None, sort: str = "newest"):
    offset = (page - 1) * 12

    query = select(model.Product.pid, model.Product.name, model.Product.description,
...
async def ProductWrite(name: str, description: str, category: list, image: str = None, price: int = 0, stock: int = 0, db: AsyncSession = None, pid: str = None, is_update: bool = False):
    try:
...
async def ProductDelete(pid: str, db: AsyncSession):
    try:
        result = await db.execute(select(model.Product).where(model.Product.pid == pid))
        product = result.scalars().first()
        if not product:
